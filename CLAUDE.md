...
**Mod menu table rule:** whenever you add, remove, or change a slider/action in `src/moddingU/modMenu.cpp`, update **both** the "Available Mods" table in `README.md` *and* the "Available Mods (current)" table directly below in this file. Keep them in sync — same numbering, same names, same min/default/max. The README is the user-facing source of truth; the CLAUDE.md mirror is so future Claude sessions can see the menu state without grepping the cpp.

**Available Mods (current):**
